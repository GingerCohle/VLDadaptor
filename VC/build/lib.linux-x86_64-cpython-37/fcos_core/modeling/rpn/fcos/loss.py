        ...
        if locations:  # Sampling in the source domain
            ...
            for l in range(len(labels)):
                ...
                if self.cfg.MODEL.MIDDLE_HEAD.PROTO_WITH_BG:
                    ...
                    neg_indx = list(
                            np.floor(np.linspace(0, len(labels[l][neg_indx]) - 2, num_pos // self.bg_ratio)))
            ...
            return pos_points, pos_labels - 1, pos_labels.new_ones(pos_labels.shape).long(), gt_reg, gt_cls, neg_points
            ...
            for l, feature in enumerate(features):
                ...
                if conf_pos_indx.any():
                    ...
                    num_pos = len(scores)
                    neg_indx_new = list(
                        np.floor(np.linspace(0, (neg_indx.sum() - 2).item(), (num_pos // self.bg_ratio))).astype(int))
            ...
            if len(pos_points) > 0:
                ...
                # plabels = torch.cat([pos_plabels])

                # loss_weight = torch.cat(pos_weight)
                return pos_points, pos_plabels - 1, pos_weight.long(), neg_points
            else:

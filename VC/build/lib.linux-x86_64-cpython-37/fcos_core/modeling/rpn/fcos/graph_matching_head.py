...
class CompactnessLoss(nn.Module):
    def __init__(self, num_branch=8, feat_dim=512):
        super(CompactnessLoss, self).__init__()
        self.num_branch = num_branch
        self.feat_dim = feat_dim
        self.centers = nn.Parameter(torch.randn(self.num_branch, self.feat_dim).cuda())

    def forward(self, feat, label, labels_exist):
        for c in labels_exist:
            c_idx = label == c
            dist = (feat[c_idx]-self.centers[c].unsqueeze(0)).pow(2).sum(dim=-1)
...
class GModule(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        # loss
        self.text_cls_loss = 1e-1
        # loss
        self.compactness_loss = CompactnessLoss(8, 512)  # numbranch随类别数变化
        self.compactness_weight = 1e-4
        ...
        self.register_buffer('sr_seed', torch.randn(self.num_classes-1, 256)) # seed = bank
        self.register_buffer('tg_seed', torch.randn(self.num_classes-1, 256))
    ...
    def _forward_train(self, images, features, targets=None, score_maps=None):
            ...
            # print(clip_crop_feat_src_batch.shape, clip_crop_label_src_batch.shape)
            # print(clip_crop_feat_src_batch, len(clip_crop_label_src_batch))
            ...
            if nodes_2 is not None:
                ...
                reg_list = []
                clip_crop_label_src_all = torch.cat(gt_cls).tolist()
                ...
                for img_v, reg_v in zip(image_list, reg_list):
                    # img_tmp = img_v
                    reg_tmp = reg_v
                    # print(cls_v)
                    preprocessed = []
                    for i in range(len(reg_tmp)):
                        ...
                        # print('*'*10)
                        if w != 0 and h != 0:
                            clip_crop_label_src_batch.append(clip_crop_label_src_all[i])
                            croped = self.preprocess(cropped_image)
                            preprocessed.append(croped)

                    preprocessed = torch.stack(preprocessed).to(self.device)
                    clip_crop_feat_src_batch.append(self.clip_model.encode_image(preprocessed))
                ...
                # print(labels_exist, set(clip_crop_label_src_batch.tolist()), set(clip_src_label.tolist()), set(clip_tgt_label.tolist()))
                if len(labels_exist) != 0:
                    ...
                    text_cls_loss = self.text_cls_loss * torch.mean(torch.cat(text_cls_loss))
                    ...
                    compact_loss = self.compactness_loss(tgt_src_jointfeat, tgt_src_jointlabel, labels_exist)
                    middle_head_loss.update({'compact_loss': self.compactness_weight * compact_loss})

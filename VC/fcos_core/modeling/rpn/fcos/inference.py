...
class FCOSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(
        ...
        box_cls = box_cls.view(N, C, H, W).permute(0, 2, 3, 1).reshape(N, -1, C)
        box_cls = box_cls.sigmoid() if self.mode == 'common' else box_cls
        ...
        pre_nms_top_n = candidate_inds.contiguous().view(N, -1).sum(1).contiguous()
        pre_nms_top_n = pre_nms_top_n.clamp(max=self.pre_nms_top_n)

        # multiply the classification scores with centerness scores
        # centerness[torch.isnan(centerness)]=0

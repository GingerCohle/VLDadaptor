...
class dot_attention(nn.Module):
    ...
    def forward(self, q, k, v, snr_mask=None, scale=None, attn_mask=None):
        ...
        if snr_mask is not None:
            attention = attention.masked_fill(snr_mask == 0, -1e9)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, key, value, query, snr_mask=None,attn_mask=None):
        ...
        if self.version == 'v2':
            ...
            context, attention = self.dot_product_attention(query, key, value, snr_mask, scale, attn_mask)

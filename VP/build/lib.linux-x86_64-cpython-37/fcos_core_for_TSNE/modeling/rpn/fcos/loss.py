...
class PrototypeComputation(object):
    ...
    def __call__(self, locations, features, targets, tgt_center):
        ...
        if locations: # Sampling in the source domain
            ...
            for l in range(len(labels)):
                ...
                level_label = labels[l].detach()*0.+ l
                ...
                pos_level_label_all = level_label[pos_indx]
                ...
                if step>1:
                    ...
                    src_level_list.append(pos_level_label_all[::step])
                else:
                    pos_points.append(pos_nodes_all)
                    pos_labels.append(pos_labels_all)
                    src_level_list.append(pos_level_label_all)
                ...
                if self.cfg.MODEL.MIDDLE_HEAD.PROTO_WITH_BG:
                    ...
                    neg_level_temp = level_label[neg_indx]
                    if len(labels[l][pos_indx]) > len(labels[l][neg_indx]):
                        ...
                        src_ng_level_list.append(neg_level_temp[neg_indx])
            pos_points = torch.cat(pos_points,dim=0)
            pos_labels = torch.cat(pos_labels,dim=0)
            pos_level_labels = torch.cat(src_level_list, dim=0)
            if self.cfg.MODEL.MIDDLE_HEAD.PROTO_WITH_BG:
                neg_points = torch.cat(neg_points, dim=0)
                neg_level_labels = torch.cat(src_ng_level_list, dim=0)
                ...
                pos_level_labels = torch.cat([neg_level_labels.long(), pos_level_labels.long()])

            return pos_points, pos_labels, pos_labels.new_ones(pos_labels.shape).long(), pos_level_labels
            ...
            for l, feature in enumerate(features):
                ...
                neg_indx = ~((act_maps > 0.05).sum(dim=-1).bool())
                # pos_level_list.append(conf_pos_indx.any().cpu().item())
                ...
                if conf_pos_indx.any():
                    ...
                    tgt_level_list.append(indx.detach() * 0. + l)
                    pos_plabels.append(indx + 1)
                    pos_weight.append(scores.detach())
                    # neg_indx = ~conf_pos_indx
                    neg_points_temp = features[l].permute(0, 2, 3, 1).reshape(-1, C)[neg_indx]
                    neg_level_temp = neg_points_temp.new_ones(neg_points_temp.size(0))*0. + l
                    num_pos = len(scores)
                    ...
                    tgt_neg_level_list.append(neg_level_temp[neg_indx_new])

            if len(pos_points)>0:
                pos_level_labels = torch.cat(tgt_level_list, dim=0)
                tgt_neg_level = torch.cat(tgt_neg_level_list, dim=0)
                pos_points = torch.cat(pos_points,dim=0)
                ...
                pos_level_labels = torch.cat([tgt_neg_level.long(), pos_level_labels.long()])

                loss_weight = torch.cat([neg_weight, pos_weight])
                return points, plabels, loss_weight.long(), pos_level_labels
            else:

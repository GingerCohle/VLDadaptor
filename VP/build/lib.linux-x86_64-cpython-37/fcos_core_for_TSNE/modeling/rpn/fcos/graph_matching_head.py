            ...
            if sr_indx.any() and tg_indx.any():  # If the category appear in both domains, we directly collect them!
                ...
                for level in tg_level_exist:
                    ...
                    if self.with_semantic_completion:
                        sr_nodes_c_level = torch.normal(0, 0.01,
                                                        size=tg_nodes_c_level.size()).cuda() + sr_nodes_c_level if len(
                            tg_nodes_c_level) < 5 \
                            else torch.normal(mean=sr_nodes_c_level, std=tg_nodes_c_level.std(0).unsqueeze(0).expand(
                            tg_nodes_c_level.size())).cuda()
                    else:
                        sr_nodes_c_level = torch.normal(0, 0.01, size=tg_nodes_c_level.size()).cuda()
                    with torch.no_grad():
                        node_feat = sr_nodes_c_level.detach()
                        bs_lvl_cls_score = F.softmax(self.node_cls_middle(node_feat), dim=-1)[:, c]
                    mean = bs_lvl_cls_score.mean()
                    std = bs_lvl_cls_score.std()
                    zscores = (bs_lvl_cls_score - mean) / std
                    threshold = 1.5
                    outlier_idx = torch.where(torch.abs(zscores) > threshold)[0]
                    mask = torch.ones(sr_nodes_c_level.size()[0]).bool().cuda()
                    if outlier_idx.bool().any():
                        mask[outlier_idx.tolist()] = False
                    sr_nodes_c_level_list.append(sr_nodes_c_level[mask])
                    tg_nodes_c_level_list.append(tg_nodes_c_level)
                    sr_level_category_level_list.append(
                        (torch.ones(tmp_len, dtype=torch.float).cuda() * level).long()[mask])
                ...
                # tgnode
                print(labels)
                sr_nodes_num = len(sr_nodes_c_level_list)
                ...
                sr_labels_category_first.append(torch.ones(sr_nodes_num, dtype=torch.float).cuda() * c)
                tg_labels_category_first.append(torch.ones(num_nodes, dtype=torch.float).cuda() * c)
                sr_weight_category_first.append(torch.ones(sr_nodes_num, dtype=torch.long).cuda())
                ...
                for level in sr_level_exist:
                    ...
                    if self.with_semantic_completion:
                        tg_nodes_c_level = torch.normal(0, 0.01,
                                                        size=tg_nodes_c_level.size()).cuda() + tg_nodes_c_level if len(
                            sr_nodes_c_level) < 5 \
                            else torch.normal(mean=tg_nodes_c_level, std=sr_nodes_c_level.std(0).unsqueeze(0).expand(
                            sr_nodes_c_level.size())).cuda()
                    else:
                        tg_nodes_c_level = torch.normal(0, 0.01, size=sr_nodes_c_level.size()).cuda()
                    with torch.no_grad():
                        node_feat = tg_nodes_c_level.detach()
                        bs_lvl_cls_score = F.softmax(self.node_cls_middle(node_feat), dim=-1)[:, c]
                    mean = bs_lvl_cls_score.mean()
                    std = bs_lvl_cls_score.std()
                    zscores = (bs_lvl_cls_score - mean) / std
                    threshold = 1.5
                    outlier_idx = torch.where(torch.abs(zscores) > threshold)[0]
                    mask = torch.ones(tg_nodes_c_level.size()[0]).bool().cuda()
                    if outlier_idx.bool().any():
                        mask[outlier_idx.tolist()] = False
                    sr_nodes_c_level_list.append(sr_nodes_c_level)
                    tg_nodes_c_level_list.append(tg_nodes_c_level[mask])
                    sr_level_category_level_list.append((torch.ones(tmp_len, dtype=torch.float).cuda() * level).long())
                    tg_level_category_level_list.append(
                        (torch.ones(tmp_len, dtype=torch.float).cuda() * level).long()[mask])
                ...
                # tgnode
                tg_nodes_num = len(tg_nodes_c_level_list)
                ...
                tg_labels_category_first.append(torch.ones(tg_nodes_num, dtype=torch.float).cuda() * c)

                sr_weight_category_first.append(sr_weight_category_level_list)
                tg_weight_category_first.append(torch.ones(tg_nodes_num, dtype=torch.long).cuda())
                sr_level_label_category_first.append(sr_level_category_level_list)
                tg_level_label_category_first.append(tg_level_category_level_list)
        ...
        with torch.no_grad():
            for cls in sr_labels.unique().long():
                ...
                for lvl in cls_lvl.unique().long():
                    ...
                    bs_lvl_cls_score = F.softmax(self.node_cls_middle(bs_lvl), dim=-1)[:, cls].unsqueeze(0).T
                    bs_lvl = torch.sum(bs_lvl * bs_lvl_cls_score, dim=0)
                    momentum_lvl = torch.nn.functional.cosine_similarity(bs_lvl.unsqueeze(0), self.sr_seed[cls][lvl].unsqueeze(0))

                    self.sr_seed[cls][lvl] = self.sr_seed[cls][lvl] * momentum_lvl + bs_lvl * (1.0 - momentum_lvl)
            ...
            if tg_nodes is not None:
                for cls in tg_labels.unique().long():
                    ...
                    for lvl in cls_lvl.unique().long():
                        ...
                        bs_lvl_cls_score = F.softmax(self.node_cls_middle(bs_lvl), dim=-1)[:, cls].unsqueeze(0).T
                        bs_lvl = torch.sum(bs_lvl * bs_lvl_cls_score, dim=0)
                        momentum_lvl = torch.nn.functional.cosine_similarity(bs_lvl.unsqueeze(0), self.tg_seed[cls][lvl].unsqueeze(0))
                        self.tg_seed[cls][lvl] = self.tg_seed[cls][lvl] * momentum_lvl + bs_lvl * (1.0 - momentum_lvl)

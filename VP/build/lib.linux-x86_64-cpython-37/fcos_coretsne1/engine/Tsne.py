...
def Hbeta_torch(D, beta=1.0):
    ...
    H = torch.log(sumP) + beta.to(D.device) * torch.sum(D * P) / sumP
    P = P / sumP
...
def x2p_torch(X, tol=1e-5, perplexity=30.0):
    ...
    beta = torch.ones(n, 1).to(X.device)
    logU = torch.log(torch.tensor([perplexity])).to(X.device)
...
def tsne(X, no_dims=2, initial_dims=50, perplexity=30.0):
    ...
    eta = 500
    min_gain = 0.01
    Y = torch.randn(n, no_dims).to(X.device)
    dY = torch.zeros(n, no_dims).to(X.device)
    iY = torch.zeros(n, no_dims).to(X.device)
    gains = torch.ones(n, no_dims).to(X.device)

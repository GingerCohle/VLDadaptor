...
def computer_featgt(features, targets, logits):
    locations = compute_locations(features)
    if locations:  # Sampling in the source domain
        N, C, _, _ = features[0].size()
        _, CLS, _, _ = logits[0].size()
        labels = prepare_targets(locations, targets)
        pos_points = []
        pos_labels = []
        pos_logits = []
        ...
        return pos_points, pos_labels, pos_logits
####
...
def foward_detector(cfg, model, images, targets=None, return_maps=True,  DA_ON=True):
    ...
    if model_fcos.training and DA_ON:
        ...
        proposals, proposal_losses, score_maps, cls_features, logits = model_fcos(
            images, features, targets=targets, return_maps=return_maps)
        if targets[0].get_field("labels").shape[0] > 1:
            node_feats, gt, logits = computer_featgt(cls_features, targets, logits)
        else:
            node_feats = None
            gt = None
            logits = None
        # node_feats = torch.nn.functional.normalize(node_feats, dim=1)
        return proposals, node_feats, gt, logits

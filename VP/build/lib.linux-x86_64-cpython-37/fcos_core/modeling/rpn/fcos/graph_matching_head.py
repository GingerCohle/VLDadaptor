...
class GModule(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        "sofa",
        "train",
        "tvmonitor",)#('person', 'car', 'train', 'rider', 'truck', 'motorcycle', 'bicycle', 'bus')
    ...
    def _forward_train(self, images, features, targets=None, score_maps=None):
        ...
        if nodes_2 is not None:
            ...
            for img_v, reg_v, cls_v in zip(image_list, reg_list, cls_list):
                img_width, img_height = img_v.size
                reg_tmp = reg_v
                cls_tmp = cls_v
                # print(cls_v)
                preprocessed = []
                for i in range(len(reg_tmp)):
                    ...
                    # x1_15 = np.clip(1.25*x1 - 0.25*x2, a_min=0,  a_max=None)
                    # y1_15 = np.clip(1.25*y1 - 0.25*y2, a_min=0,  a_max=None)
                    # x2_15 = np.clip(1.25*x2 - 0.25*x1, a_min=None, a_max=img_width)
                    # y2_15 = np.clip(1.25*y2 - 0.25*y1, a_min=None, a_max=img_height)
                    # cal center coord
                    center_x = (x1 + x2) / 2
                    center_y = (y1 + y2) / 2
                    # cal new area
                    width = x2 - x1
                    height = y2 - y1
                    new_width = int(width * self.area)
                    new_height = int(height * self.area)
                    # cal new coord
                    x1_15 = np.clip(int(center_x - new_width / 2), a_min=0,  a_max=None)
                    y1_15 = np.clip(int(center_y - new_height / 2), a_min=0,  a_max=None)
                    x2_15 = np.clip(int(center_x + new_width / 2), a_min=None, a_max=img_width)
                    y2_15 = np.clip(int(center_y + new_height / 2), a_min=None, a_max=img_height)
                    # lbl = str(cls_v[i])
                    # cv2.rectangle(img_v, (x1, y1), (x2, y2), (0, 255, 0), 2)
                    # 在目标框上方打上标签
                    # cv2.putText(img_v, lbl, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
                    # print(x1, y1, x2, y2)
                    cropped_image = img_v.crop((x1_15, y1_15, x2_15, y2_15))
